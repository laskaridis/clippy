...
- For Stimulus coordination across disjoint server-rendered roots (for example a drawer trigger outside the drawer controller root), use document-level `CustomEvent` contracts between controller owners and gate any legacy bootstrap on the presence of the migrated controller roots so old scripts do not double-bind.
